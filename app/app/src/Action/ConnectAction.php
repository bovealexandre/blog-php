...
final class ConnectAction
{
    ...
    public function __construct(TwigExtension $view, LoggerInterface $logger,$db)
    {
        $this->view = $view;
        $this->logger = $logger;
        $this->db=$db;
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        if ($pseudo === $user['pseudo'] || $pseudo === $user['email']){
          ...
          return $response->withRedirect($this->view->pathFor('home'), 301);


        }else{
          return $response->withRedirect($this->view->pathFor('connexion'), 301);
        }
        return $response;
    }
}